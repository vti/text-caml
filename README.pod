...
L<Text::Caml> is a Mustache-like (http://mustache.github.com/) template engine.
That means it tends to have no logic in template files.

=head2 Example

    my $view = Text::Caml->new;
    $view->render('template', {title => 'Hello', body => 'there!'});
...
It is possible to pass arguments too. They will replace context.
...
Normally you will want to create a separated view object for every template.
This way you can use Perl for inheritance, composition etc without bringing that
